...
const sizeOptions = [
	...
	},
	{
		...
		icon: (
			<div className="w-3 h-1.5 bg-white border border-black rounded-sm"></div>
		),
	},
	{
		value: "tall",
		...
		icon: (
			<div className="w-0.5 h-3 bg-white border border-black rounded-full"></div>
		),
	},
	{
		value: "small-square",
		label: "Small Square",
		icon: (
			<div className="w-2 h-2 bg-white border border-black rounded-sm"></div>
		),
	},
	{
		value: "large-square",
...
export function AppearanceCustomizer({
	...
	const [isUpdatingBackgroundColor, setIsUpdatingBackgroundColor] =
		useState(false);
						...
						allItems.map(async (item, index) => {
							...
							if (item.type === "link") {
								try {
									...
									console.error(
										"Failed to fetch metadata for item:",
										item.url,
										error
									);
								}
							...
							try {
								if (
									item.widget_position &&
									typeof item.widget_position === "string"
								) {
									widgetPosition = JSON.parse(item.widget_position);
								} else if (
									item.widget_position &&
									typeof item.widget_position === "object"
								) {
									widgetPosition = item.widget_position as {
										x: number;
										y: number;
									};
								}
							...
							try {
								if (
									item.web_position &&
									typeof item.web_position === "string"
								) {
									webPosition = JSON.parse(item.web_position);
								} else if (
									item.web_position &&
									typeof item.web_position === "object"
								) {
							...
							try {
								if (
									...
									typeof item.mobile_position === "string"
								) {
									mobilePosition = JSON.parse(item.mobile_position);
								} else if (
									item.mobile_position &&
									typeof item.mobile_position === "object"
								) {
									mobilePosition = item.mobile_position as {
										x: number;
										y: number;
									};
								}
							...
							return {
								...
								data: {
									...
									link_type:
										item.type === "qr_code"
											? "qr_code"
											: item.link_type || undefined,
									// For QR codes, include QR-specific data both nested and directly for compatibility
									qr_codes:
										item.type === "qr_code"
											? [
													{
														...
														format: item.format,
														size: item.qr_size || item.size, // Use qr_size for pixel dimensions, size for widget size
	...
	const handleCreateLink = async () => {
		...
		const canCreate = checkCanCreateLink(
			links,
			"link_in_bio",
			profile.tier,
			qrCodes.map((qr) => ({ is_active: true }))
		);
	...
	const handleShareLink = async () => {
		try {
			await navigator.clipboard.writeText(
				`${window.location.origin}/${currentPage?.username}`
			);
	...
	const handlePageTitleBlur = async () => {
		if (pageTitle !== (currentPage?.page_title || "") && currentPage) {
			try {
				...
				setCurrentPage((prev) =>
					prev ? { ...prev, page_title: pageTitle.trim() } : null
				);
	...
	const handlePageDescriptionBlur = async () => {
		if (
			pageDescription !== (currentPage?.page_description || "") &&
			currentPage
		) {
			try {
				const { error } = await supabase
				...
				setCurrentPage((prev) =>
					prev
						? { ...prev, page_description: pageDescription.trim() || null }
						: null
				);
	...
	const handleBackgroundColorUpdate = async (color: string) => {
		if (profile.tier && profile.tier !== "pro") {
			toast.error(
				"Background color customization is only available for Pro users."
			);
			return;
		}
	...
	const handleAddWidget = async (widget: Widget) => {
		try {
			...
			const linkTypeForCheck =
				widget.type === "qr_code" ? "qr_code" : "link_in_bio";
			const currentWidgetCount = widgets.filter(
				(w) => w.type !== "qr_code"
			).length;
			const currentLinksForCheck = [
				...links,
				...Array(currentWidgetCount).fill({ is_active: true }),
			];
			const canCreate = checkCanCreateLink(
				currentLinksForCheck,
				linkTypeForCheck,
				profile.tier,
				qrCodes.map((qr) => ({ is_active: true }))
			);
			...
			if (widget.data.platform && widget.data.username) {
				try {
					...
					if (
						...
						profileMetadata.displayName !== widget.data.username
					) {
			...
			if (
				...
				try {
					...
					if (extractedUsername) {
						...
						if (
							profileMetadata.displayName &&
							profileMetadata.displayName !== extractedUsername
						) {
							metadata.displayName = profileMetadata.displayName;
						}
					}
	...
	const handleResizeWidget = async (
		widgetId: string,
		newSize: Widget["size"]
	) => {
		try {
			...
			if (
				hasSpecialConstraints &&
				newSize !== "small-square" &&
				newSize !== "large-square"
			) {
	...
	const checkCollision = (widget1: Widget, widget2: Widget) => {
		...
		const pos1 =
			activeView === "web" ? widget1.webPosition : widget1.mobilePosition;
		const pos2 =
			activeView === "web" ? widget2.webPosition : widget2.mobilePosition;
	...
	const findValidPosition = (
		...
		const testWidget = {
			...widget,
			[activeView === "web" ? "webPosition" : "mobilePosition"]:
				snappedPosition,
		};

		// Check if position is valid (no collisions)
		const hasCollision = widgets.some(
			(w) => w.id !== widget.id && checkCollision(testWidget, w)
		);
		...
		const containerWidth =
			activeView === "mobile"
				...
				: 600; // Mobile: 320-48=272 (2 widgets of 128px + margins), Web: full width minus padding
				...
				for (let y = -distance; y <= distance; y += GRID_SIZE) {
					const candidatePosition = {
						x: Math.max(
							0,
							Math.min(
								containerWidth - getWidgetDimensions(widget.size).width,
								snappedPosition.x + x
							)
						),
					...
					const hasCollision = widgets.some(
						(w) => w.id !== widget.id && checkCollision(candidateWidget, w)
					);
	...
	const autoArrangeMobileWidgets = (widgets: Widget[]): Widget[] => {
		...
		const hasOverlaps = mobilePositions.some((pos1, i) =>
			mobilePositions.some(
				(pos2, j) => i !== j && pos1.x === pos2.x && pos1.y === pos2.y
			)
		);
	...
	const getInitialPosition = (widget: Widget) => {
		if (activeView === "mobile") {
			...
			const tempWidget = {
				...widget,
				position: initialMobilePos,
				webPosition: { x: 20, y: 20 },
				mobilePosition: initialMobilePos,
			};
	...
	const handleWidgetMove = async (
		...
		try {
			...
			const updateData = {
				...
				[activeView === "web" ? "web_position" : "mobile_position"]:
	...
	const handleDragOver = (e: React.DragEvent) => {
		...
		const mousePosition = {
			x: Math.max(
				0,
				Math.min(rect.width - 48, e.clientX - rect.left - dragOffset.x - 24)
			),
	...
	const renderWidget = (widget: Widget, inRightPanel = false) => {
		...
		const currentPosition =
			activeView === "web" ? widget.webPosition : widget.mobilePosition;
							...
							{false ? (
								...
								<>
									<Button
										size="sm"
										variant={
											widget.size === "small-square" ? "default" : "outline"
										}
										...
										onClick={() =>
											handleResizeWidget(widget.id, "small-square")
										}
									>
										Small
									</Button>
									<Button
										size="sm"
										variant={
											widget.size === "large-square" ? "default" : "outline"
										}
										className={`text-xs h-8 border-2 border-black font-medium ${
											widget.size === "large-square"
												? "bg-black text-white hover:bg-gray-800"
										...
										onClick={() =>
											handleResizeWidget(widget.id, "large-square")
										}
									>
										Large
									</Button>
								...
								<>
									...
									<Button
										size="sm"
										variant={
											widget.size === "medium-square" ? "default" : "outline"
										}
										className={`text-xs h-8 border-2 border-black font-medium ${
										...
										onClick={() =>
											handleResizeWidget(widget.id, "medium-square")
										}
									>
										Medium
									</Button>
										...
										<div className="flex items-center justify-center h-40">
											<div className="text-center text-gray-500">
												...
												<div className="text-sm">
													Use the toolbar below to add your first widget
												</div>
											</div>
										</div>
									) : (
